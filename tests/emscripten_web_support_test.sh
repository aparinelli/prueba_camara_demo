...
perl -0ne 'exit 0 if /ifneq \(\$\(findstring emcc,\$\(CC\)\),\)\s+PROJECT_LDFLAGS[[:space:]]*=.*-sASSERTIONS=1.*-sGL_ASSERTIONS=1.*-sMAX_WEBGL_VERSION=2\s+endif/s; exit 1' "$config_file"
assert_not_contains '-sMIN_WEBGL_VERSION=2|-s MIN_WEBGL_VERSION=2' "$config_file"
